...
const Chatcontent = () => {
  ...
  return (
    <div className={`w-full ${isMobile ? 'md:w-full gap-4 ': 'md:w-3/4'} md:w-[90%]  mr-20   flex md:flex-row flex-col rounded-xl mt-8`}>
      <faAngleLeft />
      <div className={`border-14  border-[10px] rounded-[1px] border-pink-400 hidden lg:block`}>
        <div className='flex flex-col items-center justify-center p-8 space-y-4'>
          <div className='flex  space-x-8 text-[#979797] font-semibold'>
            {tabNames.map((tab) => (
              <Link
                key={tab}
                // to={`/${tab}`}
      ...
      <div className={`border-[10px] ${isMobile ? 'border-black rounded-[10px]' : 'border-pink-400'}   xl:mb-0 mb-20`}>
        <div><Messagebar /></div>
        <div>
